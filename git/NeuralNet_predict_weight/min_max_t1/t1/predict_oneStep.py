...
def generateClf(type, metaDatas, week):
    ...
    clf = linear_model.LogisticRegression(multi_class='multinomial', solver='newton-cg',C=0.00004)
    #clf = linear_model.LogisticRegression(multi_class='multinomial', solver='newton-cg', C=0.00003)
...
for onePerson in testDataList:
    ...
    if abs(onePerson['weight4'] - predictWeight) < 1 :
        accurent += 1
    if ( onePerson['waistline4'] - predictWaist  ) < 3:
        accurent2 += 1
